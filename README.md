# 癌症患者诊疗周期向导

## 免责声明

**⚠️ 重要提醒**

本项目是我在陪伴母亲治疗肝门胆管癌过程中开发的。所有数据来源于个人诊疗经历和公开资料，来源于进行肿瘤治疗这个复杂过程的弯路、不解和迷茫，**不构成专业医疗意见**。

访问[http://extrame.github.io/canotion]获得该工具，直接使用即可，不会在网络上记录任何信息。

请务必以主治医生的建议为准，本工具仅供参考辅助，甄别使用。

---

## 隐私保护声明 🔒

**您的隐私数据完全本地存储**

- ✅ 所有个人健康数据（档案信息、胆红素记录、PTCD引流量等）**仅存储在您的浏览器本地**
- ✅ **不会上传到任何服务器**，包括作者的服务器
- ✅ 文章等公开内容存储在 Supabase 云端，但**不包含任何个人健康信息**

**注意事项：**
- 清除浏览器数据会导致档案丢失，建议定期备份重要数据
- 如需多设备同步，请自行导出/导入数据

---

## 项目介绍

这是一个帮助癌症患者管理诊疗周期的辅助工具，主要针对肝门胆管癌（hilar cholangiocarcinoma）患者设计。

### 主要功能

- **阶段管理**：跟踪患者从疑似诊断到治疗完成的各个阶段
- **指标监测**：记录胆红素等关键指标，提供阶段性提示
- **PTCD引流记录**：记录每日引流量，图形化展示趋势
- **任务提醒**：根据当前阶段提醒用户需要完成的事项
- **资料整理**：帮助患者整理和保存诊疗资料
- **文章中心**：提供诊疗指南等参考文章（后台可管理）

### 核心阶段

1. **疑似诊断阶段**：发现异常后的初步确认
2. **检查阶段**：完成各项检查明确病情
3. **治疗阶段**：手术、化疗等治疗方案

---

## 技术栈

- Lit + TypeScript
- Web Components
- Chart.js（数据可视化）
- LocalStorage（本地数据存储）
- Supabase（文章管理，不含个人数据）

## 运行项目

```bash
yarn install
yarn dev
```

---

## 关于肝门胆管癌

肝门部胆管癌（Perihilar cholangiocarcinoma）是胆道系统常见的恶性肿瘤，因其位置特殊、手术难度大，被称为"癌中之王"。

### 关键指标

- **胆红素**：是评估肝功能和治疗可行性的重要指标
  - < 50 μmol/L：可进行化疗
  - 51-85 μmol/L：接近安全范围
  - 85-170 μmol/L：明显升高，需关注
  - 170-200 μmol/L：重度黄疸，建议术前引流
  - > 200 μmol/L：极高危，必须先引流

### 常见治疗方法

- 手术切除
- 胆道引流（PTCD/ERCP支架）
- 化疗
- 免疫治疗
- 靶向治疗（如FGFR2抑制剂、IDH1抑制剂等）

---

## 致谢

感谢在母亲治疗过程中给予帮助的所有医护人员和家人。
